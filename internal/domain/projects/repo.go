...
func (r *Repo) ListAttachments(ctx context.Context, projectID int64) ([]Attachment, error) {
	rows, err := r.db.QueryContext(ctx, `SELECT id,project_id,request_id,filename,mime_type,size_bytes,storage_path,created_at FROM attachments WHERE project_id=? ORDER BY id DESC`, projectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&a.ID, &a.ProjectID, &rid, &a.Filename, &a.MimeType, &a.SizeBytes, &a.StoragePath, &a.CreatedAt); err != nil {
			return nil, err
		}
		if rid.Valid {
			a.RequestID = &rid.Int64
		}
		out = append(out, a)
	}
	return out, rows.Err()
...
func (r *Repo) Publish(ctx context.Context, projectID int64) error {
	_, err := r.db.ExecContext(ctx, `INSERT INTO publishing_jobs (project_id,status,error_message) VALUES (?,'succeeded','')`, projectID)
	return err
}
func (r *Repo) ListJobs(ctx context.Context, status string) ([]PublishingJob, error) {
	q := `SELECT id,project_id,store_app_id,status,error_message,created_at,updated_at FROM publishing_jobs`
	args := []any{}
	if status != "" {
		q += ` WHERE status=?`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&j.ID, &j.ProjectID, &app, &j.Status, &j.ErrorMessage, &j.CreatedAt, &j.UpdatedAt); err != nil {
			return nil, err
		}
		if app.Valid {
...
func (r *Repo) ListStoreApps(ctx context.Context, q, filter string) ([]StoreApp, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.ID, &pid, &oid, &s.Name, &s.Headline, &s.Description, &s.Visibility, &s.PublishedVersion, &s.LastPublishedAt, &s.InstallCount, &s.Featured, &s.Installed, &s.CreatedAt); err != nil {
			return nil, err
		}
		if pid.Valid {
...
func (r *Repo) GetStoreApp(ctx context.Context, id int64) (StoreApp, error) {
	...
	return StoreApp{}, sql.ErrNoRows
}
func (r *Repo) InstallApp(ctx context.Context, id int64) error {
	_, err := r.db.ExecContext(ctx, `INSERT OR IGNORE INTO installed_apps (store_app_id,user_id) VALUES (?,1)`, id)
	if err == nil {
...
func (r *Repo) GetSettings(ctx context.Context) (Settings, error) {
	var s Settings
	err := r.db.QueryRowContext(ctx, `SELECT appearance,style_profile,agent_shortcut,send_message_shortcut FROM user_settings WHERE user_id=1`).Scan(&s.Appearance, &s.StyleProfile, &s.AgentShortcut, &s.SendMessageShortcut)
	return s, err
}
func (r *Repo) SaveSettings(ctx context.Context, s Settings) error {
	_, err := r.db.ExecContext(ctx, `UPDATE user_settings SET appearance=?,style_profile=?,agent_shortcut=?,send_message_shortcut=?,updated_at=CURRENT_TIMESTAMP WHERE user_id=1`, s.Appearance, s.StyleProfile, s.AgentShortcut, s.SendMessageShortcut)
	return err
}
